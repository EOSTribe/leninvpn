...
void CoreController::initAppleController()
{
    ...
    QTimer::singleShot(0, this, [this]() { LeninVPN::toggleScreenshots(m_settings->isScreenshotsEnabled()); });

    connect(m_settings.get(), &Settings::screenshotsEnabledChanged, [](bool enabled) { LeninVPN::toggleScreenshots(enabled); });
#endif
}
